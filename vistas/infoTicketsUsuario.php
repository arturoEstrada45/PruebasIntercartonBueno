...
while ($mostrar = mysqli_fetch_array($result)) 
{
       ...
       if($estado==1){
        ...
        $result = mysqli_query($conexion, $sql);  
        header('location: ../php/envioCorreo.php?correo=' . $correo.'&soporteID='.$soporteID.'&solicitante='.$solicitante.'&asunto='.$asunto.'&ticket='.$ticketID.'&estado='.$estado);
    }
    if($estado==3){
        echo "<script> console.log($cambioEstado); </script>";
        $fechaActual=date('Y-m-d');
        $sql =  "UPDATE tickets SET estado='Pendiente' WHERE ticketID='$buscarTicket'";
        $result = mysqli_query($conexion, $sql);  
        $sql =  "UPDATE tickets SET fechaCorreccion='$fechaActual' WHERE ticketID='$buscarTicket'";
        $result = mysqli_query($conexion, $sql);  
        header('location: ../php/envioCorreo.php?correo=' . $correo.'&soporteID='.$soporteID.'&solicitante='.$solicitante.'&asunto='.$asunto.'&ticket='.$ticketID.'&estado='.$estado);
                ...
                <div class="card o-hidden border-0 shadow-lg my-5">
                    <div class="card-body p-0">
                        <!-- Nested Row within Card Body -->
                            ...
                            <?php if($imagenID!="No"){?>
                        <div class="panel-body text-center"> <img target="_blank"src="../php/cargaImgTicket.php?id=<?php echo $imagenID?>"  width="505" height="505" alt='<?php echo $imagenID?>' />   
                            ...
                            <div class="col-lg-6">
                                    ...
                                    <div class="p-5">
                                        <h1 class="h4 text-gray-900 mb-2">Descripción del Ticket </h1>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a class="btn btn-warning" href="../vistas/infoTickets.php?buscarTicket=<?php echo $buscarTicket?>&correo=<?php echo $correo?>&estado=3">Pendiente</a>
               </div>
            </div>
        </div>
    </div>
